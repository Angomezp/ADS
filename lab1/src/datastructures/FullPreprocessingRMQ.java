...
public class FullPreprocessingRMQ implements rmqInterface {
    ...
    public FullPreprocessingRMQ(int arr[]) {
        this.arr = arr;
        this.lookupTable = new int[arr.length][arr.length];
        // The extra memory usage is for the lookup table, 
        // we need to allocate a full nxn matrix.
        this.memoryBytes = (long) arr.length * arr.length * Integer.BYTES;
        preprocess();
    }
    ...
    private void preprocess() {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i+1; j < n; j++) {
                ...
                lookupTable[i][j] = (arr[lookupTable[i][j - 1]] <= arr[j]) ? lookupTable[i][j - 1] : j;
            }
        }  
    }
